...
#define DEVICE_NAME                         "MQTTSubscriber"                                        /**< Device name used in BLE undirected advertisement. */
...
void app_mqtt_evt_handler(const mqtt_client_t * p_client, const mqtt_evt_t * p_evt)
{
    ...
    switch(p_evt->id)
    {
        ...
        {
            APPL_LOG ("[APPL]: >> Data length 0x%04lX\r\n", p_evt->param.rx_param.p_data->data_len);
            APPL_LOG ("[APPL]: >> Topic length 0x%04lX\r\n", p_evt->param.rx_param.p_topic->topic_len);
            if (p_evt->param.rx_param.p_data->data_len == 1)
            {
                // Accept binary or ASCII 0 and 1.
                if((p_evt->param.rx_param.p_data->p_data[0] == 0) ||
                   (p_evt->param.rx_param.p_data->p_data[0] == 0x30))
                {
                    LEDS_OFF(LED_FOUR);
                }
                else if((p_evt->param.rx_param.p_data->p_data[0] == 1) ||
                        (p_evt->param.rx_param.p_data->p_data[0] == 0x31))
                {
                    LEDS_ON(LED_FOUR);
                }
            }
