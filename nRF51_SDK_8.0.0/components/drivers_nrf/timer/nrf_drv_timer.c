...
void nrf_drv_timer_enable(nrf_drv_timer_t const * const p_instance)
{
    ...
    nrf_timer_task_set(p_instance->p_reg, NRF_TIMER_TASKS_START);
...
void nrf_drv_timer_disable(nrf_drv_timer_t const * const p_instance)
{
    ...
    nrf_timer_task_set(p_instance->p_reg, NRF_TIMER_TASKS_SHUTDOWN);
    m_cb[p_instance->instance_id].state = NRF_DRV_STATE_INITIALIZED;
}
...
{
    ASSERT(m_cb[p_instance->instance_id].state == NRF_DRV_STATE_POWERED_ON);
    nrf_timer_task_set(p_instance->p_reg, NRF_TIMER_TASKS_STOP);
}

void nrf_drv_timer_clear(nrf_drv_timer_t const * const p_instance)
{
    ASSERT(m_cb[p_instance->instance_id].state == NRF_DRV_STATE_POWERED_ON);
    nrf_timer_task_set(p_instance->p_reg, NRF_TIMER_TASKS_CLEAR);
}
...
void nrf_drv_timer_increment(nrf_drv_timer_t const * const p_instance)
{
    ...
    nrf_timer_task_set(p_instance->p_reg, NRF_TIMER_TASKS_COUNT);
}
                               ...
                               nrf_timer_cc_channel_t        cc_channel)
{
    ...
    timer_task = (nrf_timer_tasks_t)((uint32_t)NRF_TIMER_TASKS_CAPTURE0 + (cc_channel * sizeof(uint32_t)));
    
    /*lint -save -e644*/
    nrf_timer_task_set(p_instance->p_reg, timer_task);
    /*lint -restore*/
...
uint32_t nrf_drv_timer_us_to_ticks(nrf_drv_timer_t const * const p_instance,
                                   uint32_t                      time_us)
{
    uint32_t freq_in_MHz = 16;

    freq_in_MHz   = (freq_in_MHz) / (uint32_t)(1 << (nrf_timer_frequency_get(p_instance->p_reg)));
    return (time_us * freq_in_MHz);
}
